...
public class DaTests {
    ...
    public void testPut() {
        ...
        for (char c = '!'; c <= 126; c++) {
            ...
            System.out.println(map.getTable()[hash].getKey());
            assertEquals(String.valueOf(c), map.getTable()[hash].getKey());
        }
    }
    @Test(timeout = TIMEOUT)
    public void testPut2() {
        map.put(String.valueOf("1"), Integer.valueOf("1"));
        int hash = String.valueOf("1").hashCode() % map.getTable().length;
        assertEquals(String.valueOf("1"), map.getTable()[hash].getKey());
    }
